...
typedef struct MonsterData {
	...
	int mFlags;
	unsigned char mInt;
	unsigned short mHit; // BUGFIX: changed from char to short to prevent overflow.  Fix only affects multiplayer, single player is still broken because of loading
	unsigned char mAFNum;
	unsigned char mMinDamage;
	unsigned char mMaxDamage;
	unsigned short mHit2; // BUGFIX: changed from char to short to prevent overflow.  Fix only affects multiplayer, single player is still broken because of loading
