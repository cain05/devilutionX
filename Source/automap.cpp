...
void DrawAutomapPlr()
{
	for (int i = 0; i < MAX_PLRS; i++) {
		if (i == myplr || (FriendlyMode && plr[i].plractive && plr[i].plrlevel == currlevel)) {
			...
			x = (plr[i]._pxoff * AutoMapScale / 100 >> 1) + (ScrollInfo._sxoff * AutoMapScale / 100 >> 1) + (px - py) * AmLine16 + 384;
			y = (plr[i]._pyoff * AutoMapScale / 100 >> 1) + (ScrollInfo._syoff * AutoMapScale / 100 >> 1) + (px + py) * AmLine8 + 336;
